...
void BMIH::display( void )
{
 ...
 std::cout << "biSize: " << (int) biSize << endl
...
void BMFH::display( void )
{
 ...
 std::cout << "bfType: " << (int) bfType << endl
...
RGBApixel BMP::GetPixel( int i, int j ) const
{
 ...
 {
  std::cout << "EasyBMP Warning: Attempted to access non-existent pixel;" << endl
...
bool BMP::SetColor( int ColorNumber , RGBApixel NewColor )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: Attempted to change color table for a BMP object" << endl
 ...
 if( !Colors )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: Attempted to set a color, but the color table" << endl
...
{ 
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: Attempted to access color table for a BMP object" << endl
        << "                 that lacks a color table. Ignoring request." << endl;
  }
  return Output;
 }
 if( !Colors )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: Requested a color, but the color table" << endl
        << "                 is not defined. Ignoring request." << endl;
  }
  return Output; 
 }
...
bool BMP::SetSize(int NewWidth , int NewHeight )
{
 ...
 if( NewWidth <= 0 || NewHeight <= 0 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: User attempted to set a non-positive width or height." << endl
...
bool BMP::WriteToFile( const char* FileName )
{
 ...
 if( !EasyBMPcheckDataSize() )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Error: Data types are wrong size!" << endl
 ...
 {  
  ...
  while( j > -1 )
  {
   ...
   if( !Success )
   {
    if( EasyBMPwarnings )
    {
     std::cout << "EasyBMP Error: Could not write proper amount of data." << endl;
	}
    j = -1; 
   }
   j--; 
...
{ 
 ...
 if( bmih.biCompression == 1 || bmih.biCompression == 2 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Error: " << FileName << " is (RLE) compressed." << endl
     ...
     && TempBitDepth != 24 && TempBitDepth != 32 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Error: " << FileName << " has unrecognized bit depth." << endl;
  }
  SetSize(1,1);
  SetBitDepth(1);
  fclose(fp);
 ...
 if( BitDepth < 16 )
 {
  ...
  if( NumberOfColorsToRead < TellNumberOfColors() )
  {
   if( EasyBMPwarnings )
   {
    std::cout << "EasyBMP Warning: file " << FileName << " has an underspecified" << endl
 ...
 if( BytesToSkip > 0 && BitDepth != 16 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: Extra meta data detected in file " << FileName << endl
 ...
 if( BitDepth != 16 )
 {
  ...
  while( j > -1 )
  {
   ...
   if( BytesRead < BufferSize )
   {
    ...
    if( EasyBMPwarnings )
    {
     std::cout << "EasyBMP Error: Could not read proper amount of data." << endl;
	}
   }
   else
   {
    ...
    if( !Success )
    {
     if( EasyBMPwarnings )
     {
      std::cout << "EasyBMP Error: Could not read enough pixel data!" << endl;
	 }
	 j = -1;
    }
   }   
...
bool BMP::CreateStandardColorTable( void )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   std::cout << "EasyBMP Warning: Attempted to create color table at a bit" << endl
...
void DisplayBitmapInfo( const char* szFileNameIn )
{
 ...
 std::cout << "BITMAPFILEHEADER:" << endl
 ...
 std::cout << "BITMAPINFOHEADER:" << endl
