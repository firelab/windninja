...
{
    //SF array for the volume quadrature (either(0=N,1=dN/du,2=dN/dv,3=dN/dw),local nodal point,quadrature point)
    ...
    //The SF array is split into two seperate arrays, one for the volume quadrature (SFV) and one
    //for the surface quadrature (SFS)(NOTE: SURFACE QUADRATURE NOT NEEDED BECAUSE OF PARTICULAR BCs!!!)
...
void element::computeJacobianEtc(int &elementNum, const double &u, const double &v,
                                 const double &w, double &x, double &y, double &z)
{
    ...
    DETJ=RJACV[0*3+0]*(RJACV[1*3+1]*RJACV[2*3+2]-RJACV[1*3+2]*RJACV[2*3+1])-
         RJACV[0*3+1]*(RJACV[1*3+0]*RJACV[2*3+2]-RJACV[1*3+2]*RJACV[2*3+0])+
         RJACV[0*3+2]*(RJACV[1*3+0]*RJACV[2*3+1]-RJACV[1*3+1]*RJACV[2*3+0]);

    if(DETJ<=0)
        throw std::runtime_error("Volume Jacobian 1 is zero or negative.");

    //Calculate the inverse of the Jacobian (3x3 matrix) using information from
...
{
    //Given localQuadPointNum and elementNum, function computes the Jacobian, inverse Jacobian,
    //determinant of the Jacobian, and (x,y,z)
    if(SFV == NULL)
        initializeQuadPtArrays();
    ...
    {
        DNDX[k]=RJACVI[0*3+0]*SFV[1*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum]+
                RJACVI[1*3+0]*SFV[2*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum]+
                RJACVI[2*3+0]*SFV[3*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum];

        DNDY[k]=RJACVI[0*3+1]*SFV[1*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum]+
                RJACVI[1*3+1]*SFV[2*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum]+
                RJACVI[2*3+1]*SFV[3*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum];

        DNDZ[k]=RJACVI[0*3+2]*SFV[1*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum]+
                RJACVI[1*3+2]*SFV[2*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum]+
                RJACVI[2*3+2]*SFV[3*mesh_->NNPE*NUMQPTV+k*NUMQPTV+localQuadPointNum];
    }
}

void element::computeJacobianQuadraturePoint(const int &localQuadPointNum, const int &elementNum)
{
    //Given localQuadPointNum and elementNum, function computes the Jacobian,
    //inverse Jacobian, determinant of the Jacobian, and (x,y,z)
    if(SFV == NULL)
        initializeQuadPtArrays();
...
//Given (x,y), this function locates the cell (i,j) that the point is in AND
//the internal "parent" local cell coordinates (u,v)
void element::get_uv(double const& x,double const& y, int& cell_i, int& cell_j, double& u, double &v)	
{
    int node_i, node_j;
