...
flowSeparation::flowSeparation():AsciiGrid<short>()
{
    grid_made = false;
    number_CPUs = 1;
    smalll = 1.0e-013;
...
flowSeparation::flowSeparation(Elevation const* elev, double Theta, double Phi, int number_threads):AsciiGrid<short>(elev->get_nCols(), elev->get_nRows(), elev->get_xllCorner(), elev->get_yllCorner(), elev->get_cellSize(), elev->get_noDataValue())
{	
    elevation = elev;
    theta = Theta;
    phi = Phi;
    ...
    grid_made = false;
    smalll = 1.0e-013;
    if(!compute_gridSeparation())
    {
        CPLDebug("FLOW_SEPARATION", "Could not make separation grid...");
    }else
    {
        grid_made = true;
    }
...
flowSeparation::~flowSeparation()
{
	
}

bool flowSeparation::read_separation(std::string filename)
{
    read_Grid(filename);
    grid_made = true;
    return grid_made;
...
bool flowSeparation::compute_gridSeparation()
{	
    if(!grid_made)
    {
        if(phi <= 0.0)	//if separation angle is below horizon
        {
            ...
            for(int i = 0; i < get_nRows(); i++)
            {
                ...
                {
                    data(i,j) = true; //mark as separated
                }
            }
            return true;
        }
        ...
        //compute wind "direction cosines" (negatives because this algorithm wants vector from wind to cell, not the other way how we store theta and phi)
        x_wind = -cos(thetaXY*pi/180.0);
        y_wind = -sin(thetaXY*pi/180.0);
        z_wind = -sin(phi*pi/180.0);
        ...
        // calculate absolute values for wind direction windDir
        windDirXMagnitude = x_wind;
        windDirYMagnitude = y_wind;
        if(windDirXMagnitude < 0.) windDirXMagnitude *= -1.;
        if(windDirYMagnitude < 0.) windDirYMagnitude *= -1.;

        // decide which loop will come first, the y loop or x loop
        // based on direction of wind, makes calculations faster
        if(windDirXMagnitude < windDirYMagnitude) //move in x direction first, x direction is inner loop
        {
            ...
            if(y_wind < 0.)
            {
                iX = sizeiX-1;
                diriX = -1;
                sideClosestToInlet = 3;   // 0=>west  1=>east  2=>south  3=>north
            }
            else
            {
                iX = 0;
                diriX = 1;
                sideClosestToInlet = 2;   // 0=>west  1=>east  2=>south  3=>north
            }
        }
        else //move in y direction first, y direction is inner loop
        {
            ...
            if(x_wind < 0.)
            {
                iX = sizeiX-1;
                diriX = -1;
                sideClosestToInlet = 1;   // 0=>west  1=>east  2=>south  3=>north
            }
            else
            {
                iX = 0;
                diriX = 1;
                sideClosestToInlet = 0;   // 0=>west  1=>east  2=>south  3=>north
            }
        ...
        {			
            ...
            for(iY = 0; iY < inner_loop_num; iY++)
            {
                ...
                // travel along separation angle ("ray") to determine if cell is separated or not
    ...
    {
        CPLDebug("FLOW_SEPARATION", "Separation grid has already been made...");
        return false;
    }
}

bool flowSeparation::compute_gridSeparation(Elevation const* elev, double Theta, double Phi, int number_threads)
{	
    elevation = elev;
    theta = Theta;
    phi = Phi;
    ...
    return compute_gridSeparation();
}

bool flowSeparation::track_along_ray(double px, double py, int *X, int *Y) //function moves along a path toward the inlet along the flow separation angle to determine if the cell in question is shaded
{
    ...
    while(1)
    {
        px -= x_wind;  //negative because "x_wind" is vector FROM the inlet, but we track TOWARD the inlet
        ...
        else
        {
            ...
            //check sideClosestToInlet to see which direction to move for the interpolation
            if(sideClosestToInlet==0) // 0=>west
            {
            ...
            else if(sideClosestToInlet==1)    // 1=>east
            {
                flagMap->get_cellIndex((px + 0.5), (py + 0.5 + flagMap->get_cellSize() / 2.0), &i, &j);   // The half cell subtraction is necessary for adjustment to ensure the proper cells ij
            ...
            else if(sideClosestToInlet==2)    // 2=>south
            {
            ...
            else if(sideClosestToInlet==3)    // 3=>north
            {
                flagMap->get_cellIndex((px + 0.5 - flagMap->get_cellSize() / 2.0), (py + 0.5), &i, &j);   // The half cell subtraction is necessary for adjustment to ensure the proper cells ij
        ...
        // get height of separation ray at current point while traveling along separation ray
        ...
        if(height < val)        //then it is separated
        {
            (*flagMap)(*Y,*X) = val - height;
            data(*Y,*X) = true;  //mark as separated

            break;
        }

        // if we've made it here, the pixel is attached
        (*flagMap)(*Y,*X) = -1.0f;
        data(*Y, *X) = false; //mark as attached
        break;
    }
