...
void ninja::computeDustEmissions()
{
   ...
   /*char *dstWKT = NULL;
    ...
    //=======PM10 testing=============
    /*for(int i=0;i<DustGrid.get_nRows();i++)
    {
        for(int j=0;j<DustGrid.get_nCols();j++)
        {
            if(i>60 && i< 95 && j>30 && j<90)
            {
                DustGrid(i,j) = 1;
            }
            else
            {
                DustGrid(i,j) = DustGrid.noDataValue;
            }
        }
    }
    dust.ComputePM10(UstarGrid, DustGrid);*/
    //AsciiGrid<double> DustGrid_log(DustGrid);
    /*DustGrid.write_Grid("PM10_fout", 2);

	std::string outFilename = "pm10.png";
    std::string scalarLegendFilename = "PM10_legend";
    std::string legendTitle = "PM10 Flux";
    std::string legendUnits = "(mg/m3/s)";
    bool writeLegend = TRUE;

    DustGrid.ascii2png( outFilename, scalarLegendFilename, legendUnits, legendTitle, writeLegend );


    //---------------------------------
    //dust.ComputeUstar(input, UstarGrid, u, v, w, mesh, "shearStress");
    //AsciiGrid<double> UstarGrid_shear(UstarGrid);
    ...
    //dust.ComputePM10(UstarGrid, DustGrid);
    //AsciiGrid<double> DustGrid_shear(DustGrid);
    //DustGrid.write_Grid("PM10_fout", 2);
    //=================================

    //UstarGrid = UstarGrid_log - UstarGrid_shear;
    //DustGrid = DustGrid_log - DustGrid_shear;
    //UstarGrid.write_Grid("ustar_fout", 2);
    //DustGrid.write_Grid("PM10_fout", 2);
