...
/**
 * Default constructor for wxTwo class populated with default values
 * @see wxTwo::initialize
 */
...
/**
 * Copy Constructor for wxTwo
 * @param m right hand side wxTwo object
 */
wxTwo::wxTwo( wxTwo const& m )
{
...
/**
 * Equals operator for wxTwo
 * @param m right hand side wxTwo object
 * @return reference to a new wxTwo object
 */
wxTwo& wxTwo::operator= ( wxTwo const& m )
{
    if( &m != this ) {
    stationName = m.stationName;
    lat = m.lat;
...
/**
 * Initialize a wxTwo object with default values
 */
void wxTwo::initialize()
{
    stationName = "";
    lat = lon = 0.0;
    datumType = wxTwo::WGS84;
    coordType = wxTwo::GEOGCS;
    height = zero;
...
/**
 * Fetch the height of the wxTwo.  Default height is meters
 * @param units to retrieve the height in
 * @return height of station
 */
double wxTwo::get_height( lengthUnits::eLengthUnits units, int idx )
{
    double h = height[idx];
 ...
 * @return speed
 */
double wxTwo::get_speed( velocityUnits::eVelocityUnits units, int idx )
{
    double s = speed[idx];
 ...
 */
double wxTwo::get_w_speed( velocityUnits::eVelocityUnits units)
{
    double w = w_speed;
 ...
 * @return temperature
 */
double wxTwo::get_temperature( temperatureUnits::eTempUnits units, int idx )
{
 ...
 * @return cloud cover
 */
double wxTwo::get_cloudCover( coverUnits::eCoverUnits units, int idx )
{
 ...
 */
double wxTwo::get_influenceRadius( lengthUnits::eLengthUnits units, int idx )
{
 ...
 */
void wxTwo::set_stationName( std::string Name )
{
    stationName = Name;
}
/**
 ...
 * from
 */
void wxTwo::set_location_projected( double Xord, double Yord,
                    std::string demFile )
{
    projXord = Xord;
    projYord = Yord;
 ...
 */
void wxTwo::set_location_LatLong( double Lat, double Lon,
 ...
 * @see ninjaUnits
 */
void wxTwo::set_height( double Height, lengthUnits::eLengthUnits units )
{
 ...
 * @param units
 */
void wxTwo::set_speed( double Speed, velocityUnits::eVelocityUnits units )
{
...
void wxTwo::set_direction( double Direction )
{
    direction.push_back(Direction);
}

void wxTwo::set_w_speed( double W_Speed, velocityUnits::eVelocityUnits units )
{
...
void wxTwo::set_temperature( double Temperature,
...
void wxTwo::set_cloudCover( double CloudCover,
...
void wxTwo::set_influenceRadius( double InfluenceRadius,
...
void wxTwo::set_datetime(boost::posix_time::ptime timedata)
{
    datetime.push_back(timedata);
}
boost::posix_time::ptime wxTwo::get_datetime(int idx)
{
    boost::posix_time::ptime time=datetime[idx];
    return time;
}
/** Converts vector<vector<wxStationList> > to vector<wxStation> (wxTwo for now)
...
/** Checks wxStationList for valid parameters.
 * @return True if wxStationList parameters are OK.
 */
bool wxStationList::check_station()
{
...
/**
 * I will likely get rid of the below function because the one above does everything
 * this one does+ datetime with the new header
 *
 *
 *
 */
std::vector<wxStationList> wxStationList::readStationFile( std::string csvFile,
                           std::string demFile)
{
    cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
