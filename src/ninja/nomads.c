...
try_again:
    ...
    for( i = 0; i < nFilesToGet; i++ )
    {
        ...
        pszUrl =
            CPLSPrintf( pszUrl, padfBbox[0], padfBbox[1], padfBbox[2], padfBbox[3] );
        ...
        pszFileInZip = CPLSPrintf( "/vsizip/%s/%s", pszDstZip, pszGribFile );

        fout = VSIFOpenL( pszFileInZip, "wb" );
        if( !fout )
        {
            CPLError( CE_Failure, CPLE_AppDefined,
