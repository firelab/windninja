...
void ImplicitCentralDifferenceDiffusion::Discretize() 
{
    ...
    fem.DiscretizeCentralDifferenceDiffusion(heightAboveGround, windSpeedGradient);
}
...
void ImplicitCentralDifferenceDiffusion::Initialize(const Mesh *mesh, WindNinjaInputs *input)
{
    ...
    //allocate SK, row_ptr, col_ind and initialize PHI, xRHS, yRHS, zRHS, col_ind, row_ptr
    ...
    for(k=0;k<mesh_->nlayers;k++)
    {
        for(i=0;i<input_->dem.get_nRows();i++)
        {
            for(j=0;j<input_->dem.get_nCols();j++) //loop over nodes using i,j,k notation
            {
                ...
                if(isBoundaryNode[NPK]==true)
                    xRHS[NPK]=0.; //Phi is zero on "flow through boundaries"
                    yRHS[NPK]=0.; //Phi is zero on "flow through boundaries"
                    zRHS[NPK]=0.; //Phi is zero on "flow through boundaries"
            }
        }
    }
    }	//end parallel region
 ...
 *       double check how xRHS, yRHS, zRHS, SK, and PHI are being used here
 *
 */
