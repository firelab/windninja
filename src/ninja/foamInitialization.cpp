        ...
        AsciiGrid<double>& bl_height)
{
    ...
    CPLDebug("NINJAFOAM", "input.speedInitGridFilename = %s", input.speedInitGridFilename.c_str());
    CPLDebug("NINJAFOAM", "input.dirInitGridFilename = %s", input.dirInitGridFilename.c_str());
    
    AsciiGrid<double> foamSpeedGrid, foamDirGrid;
    ...
    GDAL2AsciiGrid( (GDALDataset *)hSpeedDS, 1, foamSpeedGrid );
    GDAL2AsciiGrid( (GDALDataset *)hDirDS, 1, foamDirGrid );
    
    GDALClose(hSpeedDS);
    GDALClose(hDirDS);
    
    foamSpeedGrid.BufferGridInPlace();
    foamDirGrid.BufferGridInPlace();

    //Interpolate from input grids to dem coincident grids
    speedInitializationGrid.interpolateFromGrid(foamSpeedGrid, AsciiGrid<double>::order0);
    dirInitializationGrid.interpolateFromGrid(foamDirGrid, AsciiGrid<double>::order0);
    
    //Set windspeed grid for diurnal computation
    input.surface.set_windspeed(speedInitializationGrid);
    
    CPLDebug("NINJAFOAM", "check for coincident grids: dem = %d", speedInitializationGrid.checkForCoincidentGrids(input.dem));
    CPLDebug("NINJAFOAM", "check for coincident grids: u grid = %d", speedInitializationGrid.checkForCoincidentGrids(uInitializationGrid));
    CPLDebug("NINJAFOAM", "check for coincident grids: cloud = %d", speedInitializationGrid.checkForCoincidentGrids(cloud));
