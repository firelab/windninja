...
Application
    surfaceMeshTriangulateWithTransform
...
Description
    ...
    after all the above, options to transform the sampled output rather than just output it as is can be done
    this is effectively like running the original surfaceMeshTriangulate to create a file, then running surfaceTransformPoints on the resulting file
    both are now combined into one
...
int main(int argc, char *argv[])
{
    ...
    if (Pstream::master())
    {
        ...
        pointField points(unsortedFace.points());
        ...
        unsortedFace.movePoints(points);
