...
QString SurfaceInputView::fetchTimeZoneDetails(QString currentTimeZone) {
...
QVector<QVector<QString>> SurfaceInputView::fetchAllTimeZones(bool isShowAllTimeZonesSelected)
{
...
int SurfaceInputView::fetchDEMFile(QVector<double> boundingBox, std::string demFile, double resolution, std::string fetchType)
{
...
void SurfaceInputView::computeDEMFile(QString filePath)
{
...
double SurfaceInputView::computeMeshResolution(int index, bool isMomemtumChecked)
{
    int coarse = 4000;
    int medium = 10000;
    int fine = 20000;
...
void SurfaceInputView::computeBoundingBox(double centerLat, double centerLon, double radius, double boundingBox[4])
{
...
void SurfaceInputView::computePointRadius(double north, double east, double south, double west, double pointRadius[3])
{
    const double EARTH_RADIUS_MILES = 3958.7613;
    const double DEG_TO_RAD = M_PI / 180.0;
