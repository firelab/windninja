...
#define CLEAN_UP VSIUnlink( NOMADS_ZIP )

static int CheckBands( const char *pszZip )
{
    ...
    pszZip = CPLStrdup( CPLSPrintf( "/vsizip/%s", pszZip ) );
    papszFiles = VSIReadDir( pszZip );
    nFiles = CSLCount( papszFiles );
    for( i = 0; i < nFiles; i++ )
    {
        pszVsiFilename = CPLSPrintf( "%s/%s", pszZip, papszFiles[i] );
...
static int CheckZip( const char *pszZip )
{
    if( !EQUALN( pszZip, "/vsizip/", 8 ) )
        pszZip = CPLSPrintf( "/vsizip/%s", pszZip );
    char **papszFiles = NULL;
    papszFiles = VSIReadDir( pszZip );
    int rc = CSLCount( papszFiles );
    CSLDestroy( papszFiles );
    return rc;
}
...
BOOST_AUTO_TEST_CASE( download_1 )
{
    ...
    if( EQUAL( pszWhere, "mackay" ) )
        ...
        BOOST_REQUIRE( 0 );
    int rc = 0;
    rc = NomadsFetch( pszKey, hours, pdfBbox, NOMADS_ZIP );
    BOOST_REQUIRE_EQUAL( rc, erc );
    if( rc == 0 )
    {
        rc = CheckZip( NOMADS_ZIP );
        BOOST_CHECK_EQUAL( rc, n );
        rc = CheckBands( NOMADS_ZIP );
