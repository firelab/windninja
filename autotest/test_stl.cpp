...
BOOST_AUTO_TEST_CASE( stl_1 )
{
    ...
    rc = NinjaElevationToStl( pszFilename, "test.stl", 1, -1.0, NinjaStlBinary, NULL );
    BOOST_CHECK( !rc );
...
BOOST_AUTO_TEST_CASE( stl_2 )
{
    ...
    rc = NinjaElevationToStl( pszFilename, "test.stl", 1, -1.0, NinjaStlBinary, NULL );
    BOOST_CHECK( !rc );
    VSILFILE *fin = VSIFOpenL( "test.stl", "rb" );
    BOOST_REQUIRE( fin );
    float adfTri[12];
    double dfEdge = 0.0;
    int nTris = 0;
    ...
    rc = NinjaElevationToStl( pszFilename, "test.stl", 1, 90.0, NinjaStlBinary, NULL );
    BOOST_CHECK( !rc );
    fin = VSIFOpenL( "test.stl", "rb" );
    BOOST_REQUIRE( fin );
